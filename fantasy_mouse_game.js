...
function create() {
  ...
  initializePage(this);
  gameState.firstPage = fetchPage(1);
  displayPage(this, gameState.firstPage);
}
...
function renderCharacter(scene, key) {
  ...
  gameState.character = scene.add.image(270, 430, key).setOrigin(.5, 1).setScale(0.7);
}
...
function displayPage(scene, page) {
  ...
  for (let i=0; i<page.options.length; i++) {
    ...
    optionBox.setOrigin(0, 0).setInteractive();
